...
if z == 1
    ...
    nrcore_fit = rcore_fit/dr;
end
    

if abs(Px) <= 0.1 && abs(Py) <= 0.1
    [v_theta,Gamma_fit,rcore_fit] = vortex_fit4(-0.3,0.03,Px,Py,umesh,vmesh,xmesh,ymesh,xpos,ypos,nrcore_fit,z);
else
    Gamma_fit = Gamma_fit;
end
...
for i = 1:(zmax - averaging_frames + 1)
    avg_Gamma_fit_vect(i) = 0.2*(Gamma_fit_vect(i+1) + Gamma_fit_vect(i+2) + Gamma_fit_vect(i+3) + Gamma_fit_vect(i+4) + Gamma_fit_vect(i));
    avg_Gamma_FOV_vect(i) = 0.2*(Gamma_FOV_vect(i+1) + Gamma_FOV_vect(i+2) + Gamma_FOV_vect(i+3) + Gamma_FOV_vect(i+4) + Gamma_FOV_vect(i));
    avg_Gamma_box_vect(i) = 0.2*(Gamma_box_vect(i+1) + Gamma_box_vect(i+2) + Gamma_box_vect(i+3) + Gamma_box_vect(i+4) + Gamma_box_vect(i));
    avg_rcore_fit_vect(i) = 0.2*(rcore_fit_vect(i+1) + rcore_fit_vect(i+2) + rcore_fit_vect(i+3) + rcore_fit_vect(i+4) + rcore_fit_vect(i));
    t(i) = (i-1)*0.002;
end
...
figure;
plot(X_over_C,avg_Gamma_fit_vect,'k',X_over_C,avg_Gamma_box_vect,'b',X_over_C,avg_rcore_fit_vect,'k--');
